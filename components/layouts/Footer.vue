<template>
  <footer class="bg-gray-100 text-gray-700 py-6">
    <!-- Asosiy kontent -->
    <div class="container mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-6">
      <!-- "Biz haqimizda" bo‘limi -->
      <div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Biz haqimizda</h3>
        <ul class="space-y-1">
          <li><a href="/uz/uchastka-noktalari" class="hover:text-green-600 transition-colors">Yetkazib berish punktlari</a></li>
          <li><a href="/uz/karera" class="hover:text-green-600 transition-colors">Ish o‘rinlari</a></li>
        </ul>
      </div>

      <!-- "Foydalanuvchilar uchun" bo‘limi -->
      <div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Foydalanuvchilar uchun</h3>
        <ul class="space-y-1">
          <li><a href="#aloqa" class="hover:text-green-600 transition-colors">Biz bilan bog‘lanish</a></li>
          <li><a href="/uz/savol-javob" class="hover:text-green-600 transition-colors">Savol-javob</a></li>
        </ul>
      </div>

      <!-- "Tadbirkorlar uchun" bo‘limi -->
      <div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Tadbirkorlar uchun</h3>
        <ul class="space-y-1">
          <li><a href="https://seller.sellpoint.uz/" class="hover:text-green-600 transition-colors" rel="nofollow noopener noreferrer">SELLPOINT’da soting</a></li>
          <li><a href="https://seller.sellpoint.uz/seller/signin" class="hover:text-green-600 transition-colors" rel="nofollow noopener noreferrer">Sotuvchilar uchun kirish</a></li>
          <li><a href="/uz/promo/uchastka" class="hover:text-green-600 transition-colors" rel="nofollow noopener noreferrer">Yetkazib berish punkti ochish</a></li>
        </ul>
      </div>
    </div>

    <!-- Pastki qism -->
    <div class="container mx-auto px-4 mt-6 border-t border-gray-200 pt-4 text-center text-sm">
      <div class="flex justify-center space-x-4 mb-4">
        <!-- Ilova havolalari -->
        <a href="https://apps.apple.com/ru/app/sellpoint-internet-do-kon/id1640483056" target="_blank" rel="nofollow noopener noreferrer" class="flex items-center space-x-2 hover:text-green-600 transition-colors">
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M16.1475 0.75C16.2469 2.01122 15.8492 3.27244 15.0438 4.24338C14.2583 5.22433 13.0751 5.78488 11.8223 5.77487C11.7427 4.54367 12.1504 3.3325 12.9558 2.41161C13.7711 1.4707 14.9146 0.880126 16.1475 0.75ZM20.1292 8.17598C18.6806 9.06488 17.7915 10.623 17.7715 12.3208C17.7715 14.2385 18.9204 15.9663 20.6986 16.7154C20.3589 17.814 19.8495 18.8627 19.1801 19.8016C18.291 21.1399 17.352 22.4483 15.8635 22.4683C15.1551 22.4829 14.6784 22.2802 14.1823 22.0692C13.664 21.8488 13.1244 21.6193 12.277 21.6193C11.382 21.6193 10.8176 21.8551 10.2723 22.0828C9.80114 22.2796 9.34424 22.4705 8.70063 22.4982C7.28205 22.5582 6.20313 21.08 5.27406 19.7516C3.42592 17.045 1.98736 12.1311 3.91543 8.78523C4.82452 7.15725 6.51282 6.11854 8.38095 6.05861C9.18561 6.04161 9.95812 6.35239 10.6347 6.62457C11.1509 6.83223 11.6112 7.01742 11.9873 7.01742C12.3159 7.01742 12.7611 6.84099 13.2807 6.63506C14.1041 6.30877 15.1142 5.90844 16.1432 6.01866C17.7316 6.0686 19.2101 6.86761 20.1292 8.17598Z" />
          </svg>
          <span>AppStore</span>
        </a>
        <a href="https://play.google.com/store/apps/details?id=uz.sellpoint.app" target="_blank" rel="nofollow noopener noreferrer" class="flex items-center space-x-2 hover:text-green-600 transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M21.7756 11.5149L17.4561 9.05957L14.0156 12.5L17.4561 15.9405L21.7756 13.4852C22.541 13.0532 22.541 11.9468 21.7756 11.5149Z" fill="url(#paint0_linear)"/>
            <path d="M14.0156 12.5001L3.73969 2.22412C3.53508 2.42873 3.40625 2.70154 3.40625 3.0274V21.9727C3.40625 22.2986 3.53508 22.579 3.73969 22.776L14.0156 12.5001Z" fill="url(#paint1_linear)"/>
            <path d="M17.456 9.05959L5.10369 2.04225C4.62626 1.76943 4.08064 1.89068 3.73962 2.22412L14.0156 12.5001L17.456 9.05959Z" fill="url(#paint2_linear)"/>
            <path d="M14.0156 12.5001L3.73962 22.776C4.08064 23.117 4.62626 23.2307 5.10369 22.9579L17.456 15.9405L14.0156 12.5001Z" fill="url(#paint3_linear)"/>
            <defs>
              <linearGradient id="paint0_linear" x1="14.0156" y1="12.5" x2="22.3516" y2="12.5" gradientUnits="userSpaceOnUse">
                <stop stop-color="#FFBD00"/>
                <stop offset="1" stop-color="#FFE000"/>
              </linearGradient>
              <linearGradient id="paint1_linear" x1="14.0156" y1="12.5001" x2="3.40625" y2="23.1094" gradientUnits="userSpaceOnUse">
                <stop stop-color="#00BEFF"/>
                <stop offset="1" stop-color="#00E3FF"/>
              </linearGradient>
              <linearGradient id="paint2_linear" x1="3.40623" y1="1.89067" x2="14.0156" y2="12.5" gradientUnits="userSpaceOnUse">
                <stop stop-color="#15CF74"/>
                <stop offset="1" stop-color="#00F076"/>
              </linearGradient>
              <linearGradient id="paint3_linear" x1="14.0156" y1="12.5001" x2="3.40623" y2="23.1094" gradientUnits="userSpaceOnUse">
                <stop stop-color="#FF3A44"/>
                <stop offset="1" stop-color="#E12653"/>
              </linearGradient>
            </defs>
          </svg>
          <span>Google Play</span>
        </a>
      </div>
      <div class="flex justify-center space-x-4 mb-4">
        <!-- Ijtimoiy tarmoqlar -->
        <a href="https://www.instagram.com/sellpoint.market" target="_blank" rel="nofollow noopener noreferrer" class="hover:text-green-600 transition-colors">
          <svg class="w-5 h-5" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M24 0c20 0 24 4 24 24s-4 24-24 24S0 44 0 24 4 0 24 0Z" fill="url(#paint0_linear)"/>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M24 12.522c3.738 0 4.18.015 5.657.082 1.365.062 2.106.29 2.6.482.653.254 1.12.557 1.61 1.047.49.49.793.956 1.047 1.61.191.493.42 1.235.481 2.6.068 1.476.082 1.918.082 5.657 0 3.738-.014 4.18-.082 5.657-.062 1.365-.29 2.106-.482 2.6a4.341 4.341 0 0 1-1.046 1.61c-.49.49-.957.793-1.61 1.047-.494.191-1.235.42-2.6.481-1.476.068-1.919.082-5.657.082-3.739 0-4.181-.014-5.657-.082-1.365-.062-2.107-.29-2.6-.482a4.342 4.342 0 0 1-1.61-1.046 4.329 4.329 0 0 1-1.047-1.61c-.192-.494-.42-1.235-.482-2.6-.067-1.476-.082-1.919-.082-5.657 0-3.739.015-4.181.082-5.657.062-1.365.29-2.107.482-2.6a4.343 4.343 0 0 1 1.047-1.61 4.33 4.33 0 0 1 1.61-1.047c.493-.192 1.235-.42 2.6-.482 1.476-.067 1.918-.082 5.657-.082ZM24 10c-3.802 0-4.28.016-5.773.085-1.49.067-2.507.304-3.398.65a6.857 6.857 0 0 0-2.48 1.615 6.858 6.858 0 0 0-1.614 2.48c-.346.89-.583 1.908-.65 3.397C10.015 19.721 10 20.197 10 24c0 3.802.016 4.279.085 5.772.067 1.49.304 2.507.65 3.398.358.921.837 1.702 1.615 2.48a6.862 6.862 0 0 0 2.48 1.615c.89.346 1.908.583 3.398.65 1.493.069 1.97.085 5.772.085 3.802 0 4.28-.016 5.773-.084 1.49-.068 2.508-.305 3.398-.651a6.857 6.857 0 0 0 2.48-1.615 6.86 6.86 0 0 0 1.614-2.48c.346-.89.583-1.908.651-3.398.068-1.493.085-1.97.085-5.772 0-3.802-.017-4.28-.085-5.773-.068-1.49-.304-2.508-.65-3.398a6.855 6.855 0 0 0-1.616-2.48 6.86 6.86 0 0 0-2.48-1.614c-.89-.346-1.908-.583-3.398-.65C28.279 10.015 27.802 10 24 10Zm0 6.81a7.189 7.189 0 1 0 0 14.378 7.189 7.189 0 0 0 0-14.377Zm0 11.856a4.666 4.666 0 1 1 0-9.333 4.666 4.666 0 0 1 0 9.333Zm9.153-12.14a1.68 1.68 0 1 1-3.36 0 1.68 1.68 0 0 1 3.36 0Z" fill="#fff"/>
            <defs>
              <linearGradient id="paint0_linear" x1="42.81" y1="5.656" x2="5.044" y2="42.485" gradientUnits="userSpaceOnUse">
                <stop stop-color="#BA00B2"/>
                <stop offset=".5" stop-color="#F40000"/>
                <stop offset="1" stop-color="#FFA800"/>
              </linearGradient>
            </defs>
          </svg>
        </a>
        <a href="https://t.me/sellpoint_market" target="_blank" rel="nofollow noopener noreferrer" class="hover:text-green-600 transition-colors">
          <svg class="w-5 h-5" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M24 0c20 0 24 4 24 24s-4 24-24 24S0 44 0 24 4 0 24 0Z" fill="url(#paint0_linear)"/>
            <path d="M11.792 23.799c6.996-3.048 11.662-5.058 13.996-6.029 6.665-2.772 8.05-3.254 8.953-3.27.198-.003.642.046.93.28.242.196.31.463.341.65.032.186.072.612.04.944-.36 3.795-1.924 13.005-2.719 17.255-.336 1.798-.998 2.401-1.64 2.46-1.394.129-2.452-.92-3.802-1.806-2.112-1.384-3.305-2.246-5.356-3.597-2.37-1.562-.833-2.42.517-3.823.354-.367 6.494-5.952 6.613-6.459.015-.063.029-.3-.111-.424-.14-.125-.348-.082-.497-.048-.212.048-3.587 2.278-10.124 6.69-.958.659-1.825.979-2.602.962-.857-.018-2.506-.484-3.731-.883-1.503-.488-2.698-.747-2.594-1.576.054-.432.65-.874 1.786-1.326Z" fill="#fff"/>
            <defs>
              <linearGradient id="paint0_linear" x1="24" y1="0" x2="24" y2="47.644" gradientUnits="userSpaceOnUse">
                <stop stop-color="#2AABEE"/>
                <stop offset="1" stop-color="#229ED9"/>
              </linearGradient>
            </defs>
          </svg>
        </a>
        <a href="https://www.facebook.com/sellpointmarket" target="_blank" rel="nofollow noopener noreferrer" class="hover:text-green-600 transition-colors">
          <svg class="w-5 h-5" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M24 0c20 0 24 4 24 24s-4 24-24 24S0 44 0 24 4 0 24 0Z" fill="#1877F2"/>
            <path d="M33.186 30.938 34.25 24h-6.656v-4.502c0-1.898.93-3.748 3.911-3.748h3.026V9.844s-2.746-.469-5.372-.469c-5.482 0-9.065 3.322-9.065 9.337V24H14v6.938h6.094v17.009C21.5 48 21.5 48 23.844 48c1.276 0 2.656 0 3.75-.053v-17.01h5.592Z" fill="#fff"/>
          </svg>
        </a>
        <a href="https://www.youtube.com/channel/UCY3nNF2MUDKHrELA6LzbnHA" target="_blank" rel="nofollow noopener noreferrer" class="hover:text-green-600 transition-colors">
          <svg class="w-5 h-5" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M24 0c20 0 24 4 24 24s-4 24-24 24S0 44 0 24 4 0 24 0Z" fill="red"/>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M36.502 13.657a3.972 3.972 0 0 1 2.83 2.778C40 18.886 40 24 40 24s0 5.114-.669 7.565a3.972 3.972 0 0 1-2.829 2.778C34.006 35 24 35 24 35s-10.007 0-12.502-.657a3.973 3.973 0 0 1-2.83-2.778C8 29.114 8 24 8 24s0-5.114.669-7.565a3.973 3.973 0 0 1 2.829-2.778C13.992 13 24 13 24 13s10.006 0 12.502.657ZM29 24l-8 4.75v-9.5L29 24Z" fill="#fff"/>
          </svg>
        </a>
      </div>
      <div>
        <a href="https://legal.sellpoint.uz/maxfiylik-siyosati.html" class="hover:text-green-600 transition-colors mr-4" rel="nofollow noopener noreferrer">Maxfiylik siyosati</a>
        <a href="https://legal.sellpoint.uz/foydalanuvchi-shartnomasi.html" class="hover:text-green-600 transition-colors" rel="nofollow noopener noreferrer">Foydalanuvchi shartnomasi</a>
        <p class="mt-2">«2025© SELLPOINT MCHJ». INN 309376127. Barcha huquqlar himoyalangan</p>
      </div>
    </div>
  </footer>
</template>

<script>
export default {};
</script>

<style>
/* Tailwind CSS loyiha konfiguratsiyasi orqali qo‘llaniladi */
</style>